...
#import "PublishDanVC.h"
#import "PublishAbleDanVC.h"
#import "TabBarController.h"

@interface PublishVC ()<UIAlertViewDelegate>
        ...
        if (responseObject.key.intValue == 1) {
            ...
            for (LxmHomeBannerModel * model in arr) {
                if (model.type.intValue == 2) {
                    [self.arr1 lxm_add1Object:model];
                }else if (model.type.intValue == 4){
                     [self.arr2 lxm_add1Object:model];
                }
            }
            ...
            self.headerView.frame = CGRectMake(0, 0, ScreenW, CGRectGetMaxY(self.itemView2.frame));
        }else{
...
- (void)publishWithItem:(NSInteger)item{
    if ([LxmTool ShareTool].userModel.type == 4) {
        LxmHomeBannerModel * model = [self.arr1 lxm_object1AtIndex:item];
        PublishDanVC * vc = [[PublishDanVC alloc] initWithTableViewStyle:UITableViewStyleGrouped LxmHomeBannerModel:model];
...
- (void)ablePublishWithItem:(NSInteger)item{
    ...
    if ([LxmTool ShareTool].userModel.type == 4) {
        LxmHomeBannerModel * model = [self.arr2 lxm_object1AtIndex:item];
        PublishAbleDanVC * vc = vc = [[PublishAbleDanVC alloc] initWithTableViewStyle:UITableViewStyleGrouped LxmHomeBannerModel:model];
        [self.navigationController pushViewController:vc animated:YES];
    }else if ([LxmTool ShareTool].userModel.type == 3) {
        [SVProgressHUD showErrorWithStatus:@"还未实名认证"];
    }else {
