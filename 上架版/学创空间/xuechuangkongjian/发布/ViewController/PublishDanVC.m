...
@interface PublishDanVC ()<LxmPublishImgViewDelegate,UITextFieldDelegate,LxmLeftLabRightNumViewDelegate,UITextViewDelegate>
...
- (void)publishBtnClick{
    ...
    if (self.moneyView.numTF.text.floatValue == 0) {
    ...
    [LxmNetworking NetWorkingUpLoad:[LxmURLDefine user_releaseHelp] images:self.imgs parameters:dict name:@"img" success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"key"] intValue] == 1) {
            PayVC * vc = [[PayVC alloc] initWithTableViewStyle:UITableViewStylePlain type:1];
...
- (void)initHeaderView{
    ...
    self.moneyView.numTF.keyboardType = UIKeyboardTypeDecimalPad;
    self.moneyView.delegate = self;
    self.moneyView.isFloat = YES;
...
- (void)numClick:(UIButton *)btn{
    if (btn == self.desBtn) {
        ...
        if (self.isFloat) {
            CGFloat num  = self.numTF.text.floatValue;
            if (num>1) {
                num = num-1;
                self.numTF.text = [NSString stringWithFormat:@"%0.2f",num];
            }else  if (num==0){
                self.numTF.text = [NSString stringWithFormat:@"%@",@0];
        ...
        //加
        if (self.isFloat) {
            if (self.numTF.text.floatValue < 9999) {
                CGFloat num = self.numTF.text.floatValue;
                num = num+1;
                self.numTF.text = [NSString stringWithFormat:@"%0.2f",num];
            }
