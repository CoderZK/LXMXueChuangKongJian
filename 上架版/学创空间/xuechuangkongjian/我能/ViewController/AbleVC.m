...
@interface AbleVC ()<UIScrollViewDelegate,LxmGoodListTopViewDelegate,SNYQRCodeVCDelegate>
@property (nonatomic , strong)LxmGoodListTopView * topView;
@property (nonatomic , strong)UIScrollView * scrollView;
@property (nonatomic , strong)SubAbleVC * ableVC;
        ...
        [LxmNetworking networkingPOST:[LxmURLDefine user_findSkillTypeList] parameters:nil returnClass:[LxmGoodListRootModel class] success:^(NSURLSessionDataTask *task, LxmGoodListRootModel * responseObject) {
            if (responseObject.key.intValue == 1) {
                ...
                for (int i = 0; i<tempArr.count; i++) {
                    ...
                    SubAbleVC *ableVC = [[SubAbleVC alloc] initWithTableViewStyle:UITableViewStylePlain];
                    ...
                    ableVC.preVC = self;
                    if (i == 0) {
                        LxmGoodListModel * model = [tempArr lxm_object1AtIndex:0];
                        ableVC.currentModel = model;
                    }
                    [self.scrollView addSubview:ableVC.view];
                    [self addChildViewController:ableVC];
                    [self.ableVCArr lxm_add1Object:ableVC ];
                }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.scrollView) {
        ...
        if (toN >= 0 && toN < self.totalArr.count &&  toN < self.ableVCArr.count) {
            ...
            if (self.currentIndex != toN) {
                ...
                SubAbleVC * ableVC = [self.ableVCArr lxm_object1AtIndex:toN];
                ableVC.currentModel = model;
...
- (void)LxmGoodListTopView:(LxmGoodListTopView *)topView btnAtIndex:(NSInteger)index {
    if (index >= 0 && index < self.totalArr.count &&  index < self.ableVCArr.count) {
        ...
        if (self.currentIndex != index) {
            ...
            SubAbleVC * ableVC = [self.ableVCArr lxm_object1AtIndex:index];
            ableVC.currentModel = model;
