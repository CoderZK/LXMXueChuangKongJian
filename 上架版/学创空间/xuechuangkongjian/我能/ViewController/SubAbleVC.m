...
@interface SubAbleVC ()<LxmGoodListCellDelegate>
    ...
    if (self.isyouke) {
        ...
        [LxmNetworking networkingPOST:str parameters:dict returnClass:[LxmCanListRootModel class] success:^(NSURLSessionDataTask *task, LxmCanListRootModel * responseObject) {
            ...
            if (responseObject.key.intValue == 1) {
                ...
                NSArray *hideArr = [NSUserDefaults.standardUserDefaults objectForKey:@"hideArr"];
                if (hideArr.count > 0) {
                    NSArray *tmpArr = self.dataArr.copy;
                    for (LxmCanListModel *m in tmpArr) {
                        NSString *key = [NSString stringWithFormat:@"woneng-%@", m.billId];
                        if ([hideArr containsObject:key]) {
