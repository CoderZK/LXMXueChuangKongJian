...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LxmCanListModel * model = [self.dataArr lxm_object1AtIndex:indexPath.row];
    GoodsDetailVC * vc = nil;
    if (model.state.intValue == 1) {
        //在售
        vc = [[GoodsDetailVC alloc] initWithTableViewStyle:UITableViewStyleGrouped type:GoodsDetailVC_type_ableDetail];
    }else{
        //下架
        vc = [[GoodsDetailVC alloc] initWithTableViewStyle:UITableViewStyleGrouped type:GoodsDetailVC_type_myJineng];
    }
    vc.billId = model.billId;
    vc.model = model;
    vc.isOwn = YES;
