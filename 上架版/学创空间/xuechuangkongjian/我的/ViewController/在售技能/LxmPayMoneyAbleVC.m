...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LxmCanListModel * model = [self.dataArr lxm_object1AtIndex:indexPath.row];
    if (self.type == LxmPayMoneyAbleVC_type_fbjn) {
        model.isfbjn = YES;
    }
...
- (void)LxmPayMoneyAbleCell:(LxmPayMoneyAbleCell *)cell btnAtIndex:(NSInteger)index{
    ...
    LxmCanListModel * model = [self.dataArr lxm_object1AtIndex:indexP.row];
    if (index == 20) {//左
        if (self.type == LxmPayMoneyAbleVC_type_fbjn) {
...
- (void)payWithOrderID:(NSString *)orderID money:(NSNumber *)money withtype:(NSInteger)type{
    PayVC * vc = nil;
    if (type == 3) {
        vc = [[PayVC alloc] initWithTableViewStyle:UITableViewStylePlain type:3];
    }else{
        vc = [[PayVC alloc] initWithTableViewStyle:UITableViewStylePlain type:4];
    }
    __weak typeof(self) safe_self = self;
    vc.refreshPreVC = ^{
...
- (void)setModel:(LxmCanListModel *)model{
    ...
    NSAttributedString *stt = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"￥%0.2f",model.allMoney.floatValue] ? [NSString stringWithFormat:@"￥%0.2f",model.allMoney.floatValue] : @"￥0" attributes:@{NSForegroundColorAttributeName:YellowColor}];
