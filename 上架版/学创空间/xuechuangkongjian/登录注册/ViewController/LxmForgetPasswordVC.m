    ...
    if (ISLOGIN) {
        ...
        [LxmNetworking networkingPOST:[LxmURLDefine user_getAccountInfo] parameters:@{@"token":SESSION_TOKEN} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            if ([responseObject[@"key"] intValue] == 1) {
                selfWeak.accountModel = [LxmAccountInfoModel mj_objectWithKeyValues:responseObject[@"result"]];
                selfWeak.passwordView.phoneTF.text = self.accountModel.phone;
                selfWeak.passwordView.phoneTF.userInteractionEnabled = NO;
            }else{
