...
@interface GoodsDetailVC ()<PopListViewDelegate,LxmTaskBottomViewDelegate,LxmTaskDetailHeaderViewDelegate,LxmTaskDetailCellDelegate,UMSocialShareMenuViewDelegate>

@property (nonatomic , strong) PopListView * popListView;
@property (nonatomic , assign) NSInteger currentIndex;

@property (nonatomic , assign) GoodsDetailVC_type type;
...
- (void)viewDidLoad {
    ...
    if (self.type == GoodsDetailVC_type_goodsDetail) {
        //电商
        ...
        if (self.type == GoodsDetailVC_type_myJineng) {
    ...
    if (LxmTool.ShareTool.isShenHe) {
        self.popListView = [[PopListView alloc] initWithTitleArr:@[@"分享",@"举报",@"拉黑"] currectIndex:_currentIndex isAccShow:NO];
    } else {
...
-(void)PopListView:(PopListView *)view  didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (self.type == GoodsDetailVC_type_goodsDetail) {
            vc.billId = self.model.ecId.stringValue.length > 0 ? self.model.ecId : self.billId;
        }else if (self.type == GoodsDetailVC_type_ableDetail){
            vc.billId = self.model.billId.stringValue.length > 0 ? self.model.billId : self.billId;
        };
        
        [self.navigationController pushViewController:vc animated:YES];
    } else if (indexPath.row == 2) {
//        NSMutableArray *arr = [NSMutableArray array];
//        NSArray*tmpArr= [[NSUserDefaults standardUserDefaults] objectForKey:@"hideArr"];
//        if (tmpArr.count > 0) {
//            [arr addObjectsFromArray:tmpArr];
//        }
//        if (self.type == GoodsDetailVC_type_goodsDetail) {
//            NSString *key = [NSString stringWithFormat:@"goods-%@", self.model.ecId.stringValue.length > 0 ? self.model.ecId : self.billId];
//            [arr lxm_add1Object:key];
//        }else if (self.type == GoodsDetailVC_type_ableDetail){
//            NSString *key = [NSString stringWithFormat:@"woneng-%@", self.model.billId.stringValue.length > 0 ? self.model.billId : self.billId];
//            [arr lxm_add1Object:key];
//        };
//        [NSUserDefaults.standardUserDefaults setObject:arr forKey:@"hideArr"];
//        [LxmEventBus sendEvent:@"hideArrChanged" data:arr];
//        [self.navigationController popViewControllerAnimated:YES];
        
        if (self.detailModel.relUserId.intValue == LxmTool.ShareTool.userModel.userId) {
                   [SVProgressHUD showErrorWithStatus:@"自己不能拉黑自己！"];
                   return;
               }
        [SVProgressHUD show];
        [LxmNetworking networkingPOST:user_shield parameters:@{@"shield_userId":self.detailModel.relUserId,@"token":SESSION_TOKEN} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            [SVProgressHUD dismiss];
            if ([responseObject[@"key"] intValue] == 1) {
                [LxmEventBus sendEvent:@"yilahei" data:nil];
                [SVProgressHUD showSuccessWithStatus:@"已拉黑此贴用户！"];
                [self.navigationController popViewControllerAnimated:YES];
            } else {
                [UIAlertController showAlertWithKey:responseObject[@"key"] message:responseObject[@"message"]];
            }
        ...
        if (index == 0) {
            ...
            if (ISLOGIN) {
                ...
                if (self.isOwn == NO && self.model.relUserId.intValue != [LxmTool ShareTool].userModel.userId) {
                    YueDanPayVC * vc  = [[YueDanPayVC alloc] initWithTableViewStyle:UITableViewStyleGrouped detailModel:self.detailModel];
                    vc.refreshPreVC = ^{
                        [self loadDetailData];
                    };
                    if (self.type == GoodsDetailVC_type_goodsDetail) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.type == GoodsDetailVC_type_goodsDetail) {
        ...
        LxmCommentReplyListModel * model  = [sectionModel.replyList lxm_object1AtIndex:indexPath.row];
        cell.delegate = self;
        cell.model = model;
        return cell;
    }else{
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                cell.imgStr = [self.imgs lxm_object1AtIndex:indexPath.row - 1];
                return cell;
            }
    ...
    if (self.type == GoodsDetailVC_type_goodsDetail) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                NSString *imgStr = [self.imgs lxm_object1AtIndex:indexPath.row - 1];
...
- (void)LxmTaskDetailHeaderView:(LxmTaskDetailHeaderView *)headerView zanIndex:(NSInteger)index{
    LxmCommentListModel * model = [self.dataArr lxm_object1AtIndex:index];
    [self dianzanwithType:@2 withMain:model withReplyModel:nil];
    NSLog(@"%ld",index);
}
- (void)LxmTaskDetailCellZanClick:(LxmTaskDetailCell *)cell{
    NSIndexPath * indexP = [self.tableView indexPathForCell:cell];
    LxmCommentListModel * sectionModel = nil;
    if (self.type == GoodsDetailVC_type_goodsDetail) {
        sectionModel = self.dataArr[indexP.section];
     
    }else{
        sectionModel = self.dataArr[indexP.section-1];
    }
    LxmCommentReplyListModel * model  = [sectionModel.replyList lxm_object1AtIndex:indexP.row];
...
- (void)setInfoModel:(LxmGoodsDetailModel *)infoModel{
    ...
    if (_infoModel.money.intValue == _infoModel.money.doubleValue) {
       self.moneyLab.text = [NSString stringWithFormat:@"%d元/%@   %@",infoModel.money.intValue,infoModel.unit,[NSString returnTypeNameWithType:infoModel.sendType.intValue]];
    } else {
       self.moneyLab.text = [NSString stringWithFormat:@"%.2f元/%@   %@",infoModel.money.doubleValue,infoModel.unit,[NSString returnTypeNameWithType:infoModel.sendType.intValue]];
    }
