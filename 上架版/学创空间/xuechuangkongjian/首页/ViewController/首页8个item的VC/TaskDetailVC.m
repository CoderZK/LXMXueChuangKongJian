...
@interface TaskDetailVC ()<PopListViewDelegate,LxmTaskBottomViewDelegate,MLYPhotoBrowserViewDataSource,LxmTaskDetailHeaderViewDelegate,LxmTaskDetailCellDelegate,LxmQiangDanAlertViewDelegte,UMSocialShareMenuViewDelegate>
...
//        }
//        NSString *key = [NSString stringWithFormat:@"woneng-%@", self.billId.stringValue];
...
- (void)loadShareData{
    ...
    [LxmNetworking networkingPOST:[LxmURLDefine app_shareInfo] parameters:@{@"type":@1,@"id":self.billId,@"deviceType":@"2"} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"key"] intValue] == 1) {
            //分享
