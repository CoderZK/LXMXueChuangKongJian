...
@interface XckjLvAndPaiMaiDetailVC ()<UIScrollViewDelegate,PopListViewDelegate,LxmTaskBottomViewDelegate,LxmPaiMaiAndZiXunBottomViewDelegate,LxmTaskDetailCellDelegate,LxmTaskDetailHeaderViewDelegate,LxmLvAndPaiMaiItemViewDelegate,UMSocialShareMenuViewDelegate>
...
@implementation XckjLvAndPaiMaiDetailVC
            ...
            if (responseObject.key.intValue == 1) {
                ...
                if (self.detailModel.pic&&![self.detailModel.pic isEqualToString:@""]) {
                    ...
                    for (int i = 0; i<arr.count; i++) {
                        ...
                        model.pic = arr[i];
                        [bannerArr lxm_add1Object:model];
                    }
...
- (void)loadShareData{
    ...
    [LxmNetworking networkingPOST:[LxmURLDefine app_shareInfo] parameters:@{@"type":@3,@"id":self.typeModel == nil ? self.textId:self.typeModel.articleId ,@"deviceType":@"2"} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"key"] intValue] == 1) {
            //分享
...
- (void)lxmTaskBottomView:(LxmTaskBottomView *)bottomView btnAtIndex:(NSInteger)index{
    if (index == 0) {
        ...
        vc.articleID = self.textId.length > 0 ? @(self.textId.intValue): self.typeModel.articleId;
