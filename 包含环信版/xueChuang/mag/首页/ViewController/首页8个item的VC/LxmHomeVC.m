...
#import "LxmQiangDanVC.h"
#import "LxmLvYouAndPaiMaiVC.h"
#import "LxmAroundVC.h"
...
@interface LxmHomeVC ()<SNY_AdScrollViewDelegate,LxmHomeNewCellDelegate,SNYQRCodeVCDelegate,SNY_AdScrollViewDelegate>
    ...
    if (ISLOGIN) {
        [LxmNetworking networkingPOST:[LxmURLDefine user_findMuduleList] parameters:@{@"token":SESSION_TOKEN} returnClass:[LxmHomeBannerRootModel class] success:^(NSURLSessionDataTask *task, LxmHomeBannerRootModel * responseObject) {
            if (responseObject.key.intValue == 1) {
                ...
                self.cycleScrollView.frame = CGRectMake(15, CGRectGetMaxY(self.gudingImgView.frame) + 10, ScreenW - 30, 150);
...
- (void)initHeaderView{
    ...
    WeakObj(self);
    self.headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenW, 100 + 150 + (ScreenW*180/750) + 15)];
    ...
    //轮播图
    self.cycleScrollView = [[SNY_AdScrollView alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(self.gudingImgView.frame) + 10, ScreenW - 30, 150)];
...
- (void)diansahngClick{
    LxmGoodListVC * vc = [[LxmGoodListVC alloc] init];
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
...
-(void)sny_adScrollView:(SNY_AdScrollView *)scrollView selectedIndex:(NSInteger)index {
    ...
    LxmHomeBannerModel * model = [self.cycleScrollViewModelArray objectAtIndex:index];
    if (model.type.intValue == 1) {
        [self loadBannerDetailWith:model.bannerId];
    }else if (model.type.intValue == 2){
        //文章
        if ([model.typeId intValue] == 9) {
            LxmAroundDetailVC *aroundDetailViewController = [[LxmAroundDetailVC alloc] init];
            ...
            LxmLvAndPaiMaiDetailVC *  vc = [[LxmLvAndPaiMaiDetailVC alloc] initWithTableViewStyle:UITableViewStyleGrouped typeModel:nil type:model.typeId];
        ...
        //电商
        LxmGoodsDetailVC *goodDetailViewController = [[LxmGoodsDetailVC alloc] initWithTableViewStyle:UITableViewStyleGrouped type:LxmGoodsDetailVC_type_goodsDetail];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    LxmHomeModel * model = [_dataArr objectAtIndex:indexPath.row];
    return model.height;
}
...
- (void)toPageWith:(LxmHomeBannerModel *)model{
    ...
    if (ISLOGIN) {
        if (model.type.intValue == 2) {
            LxmQiangDanVC * vc  = vc = [[LxmQiangDanVC alloc] initWithTableViewStyle:UITableViewStylePlain currentModel:model];
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:YES];
        }else if (model.type.intValue == 3){
            if (model.ID.intValue == 9) {
                LxmAroundVC * vc = [[LxmAroundVC alloc] initWithTableViewStyle:UITableViewStylePlain type:LxmAroundVC_type_home];
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
            }else{
                LxmLvYouAndPaiMaiVC * vc = [[LxmLvYouAndPaiMaiVC alloc] initWithTableViewStyle:UITableViewStylePlain currentModel:model];
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
            }
        }
...
- (void)LxmHomeNewCellZanClick:(LxmHomeCell *)cell{
    ...
    LxmHomeModel * model = [_dataArr objectAtIndex:indexP.row];
    LxmTaskDetailVC * detailVC = [[LxmTaskDetailVC alloc] init];
    detailVC.billId = model.billId;
    detailVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:detailVC animated:YES];
