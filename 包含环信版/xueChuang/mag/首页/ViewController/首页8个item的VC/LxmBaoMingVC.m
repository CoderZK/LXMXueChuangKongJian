...
@interface LxmBaoMingVC ()<UITextFieldDelegate>
    ...
    if (SESSION_TOKEN) {
        ...
        [LxmNetworking networkingPOST:[LxmURLDefine user_insertTourOrder] parameters:dic returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            if ([responseObject[@"key"] intValue] == 1) {
                LxmPayVC * vc = [[LxmPayVC alloc] initWithTableViewStyle:UITableViewStylePlain type:2];
...
- (void)setModel:(LxmArticleDetailModel *)model{
    ...
    self.moneylab.text = [NSString stringWithFormat:@"￥%d",model.money.intValue];
}
...
- (void)setDetailModel:(LxmMyTourListModel *)detailModel{
    ...
    self.moneylab.text = [NSString stringWithFormat:@"￥%@",detailModel.money];
