...
@interface LxmAroundDetailVC ()<PopListViewDelegate,UIScrollViewDelegate,LxmAroundDetailTopItemViewDelegate,UMSocialShareMenuViewDelegate>
...
@implementation LxmAroundDetailVC
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LxmAroundCommentModel * model = [self.dataArr objectAtIndex:indexPath.row];
    cell.model = model;
...
- (void)bottomBtnClick{
    if (ISLOGIN) {
        ...
        LxmPingJiaVC * vc = [[LxmPingJiaVC alloc] initWithTableViewStyle:UITableViewStylePlain model:self.detailModel];
        vc.refreshPreVC = ^{
            [self loadDetailData];
            self.time = [NSString getCurrentTimeChuo];
            self.page = 1;
...
- (void)loadShareData{
    ...
    [LxmNetworking networkingPOST:[LxmURLDefine app_shareInfo] parameters:@{@"type":@4,@"id":self.nearId} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"key"] intValue] == 1) {
            //分享
...
- (void)setModel:(LxmAroundModel *)model{
    ...
    self.numLab.text = [NSString stringWithFormat:@"￥%d/人",model.money.intValue];
