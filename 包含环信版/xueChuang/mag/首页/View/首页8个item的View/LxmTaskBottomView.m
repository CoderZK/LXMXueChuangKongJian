...
//约单时的聊一聊
@property (nonatomic , strong) UIButton * yuedanBtn;
...
- (instancetype)initWithFrame:(CGRect)frame withStyle:(LxmTaskBottomView_style)style
{
    ...
    if (self) {
        ...
        [self.bottomView addSubview:self.yuedanBtn];
        [self setConstrains];
        ...
        if (style == LxmTaskBottomView_style_yudan) {
            self.yuedanBtn.hidden = NO;
        }else{
            self.yuedanBtn.hidden = YES;
        }
...
- (void)setConstrains{
    ...
    [self.yuedanBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.trailing.equalTo(self.baoMingBtn.mas_leading);
...
- (UIButton *)yuedanBtn{
    if (!_yuedanBtn) {
        _yuedanBtn = [[UIButton alloc] init];
        _yuedanBtn.titleLabel.numberOfLines = 0;
        _yuedanBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
        [_yuedanBtn setBackgroundImage:[UIImage imageNamed:@"orange"] forState:UIControlStateNormal];
        [_yuedanBtn setTitle:@"聊一聊" forState:UIControlStateNormal];
        [_yuedanBtn setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
        _yuedanBtn.tag = 3;
        [_yuedanBtn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _yuedanBtn;
}
...
- (void)setModel:(LxmArticleDetailModel *)model{
    ...
    NSString * ss = [NSString stringWithFormat:@"报名:￥%@",model.money];
