...
- (void)newFriendClick{
    ...
    LxmNewFriendVC *vc = [[LxmNewFriendVC alloc] init];
...
- (void)addFriendClick {
    LxmAddFriendVC *vc = [[LxmAddFriendVC alloc] init];
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *key = self.keyArr[indexPath.section];
    LxmNewFriendListModel *model = self.dataDict[key][indexPath.row];
    NSMutableArray * idArr = [NSMutableArray arrayWithArray:@[[NSString stringWithFormat:@"%d",model.friendId.intValue],myID]];
    ...
    [LxmNetworking networkingPOST:[LxmURLDefine user_getUserInfoFromHX] parameters:@{@"token":SESSION_TOKEN,@"userIds":ids} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"key"] intValue] == 1) {
            ...
            EaseMessageViewController *chatController = [[EaseMessageViewController alloc] initWithConversationChatter:[NSString stringWithFormat:@"%d",model.friendId.intValue] conversationType:EMConversationTypeChat];
