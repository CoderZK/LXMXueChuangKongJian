            ...
            if (responseObject.key.intValue == 1) {
                ...
                if (model.list.count> 0) {
                    ...
                    for (int i = 0 ; i < model.list.count; i++){
                        [self.didSelectedBoolArray addObject:[NSNumber numberWithBool:NO]];
                    }
                }
            }else{
...
- (void)next{
    ...
    for (LxmMyQingDanPeopleModel *model in self.dataArr) {
        if (model.isSelect) {
            [selectIDs addObject:model.allotId];
            [names addObject:model.robUserNick];
        }
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.model = model;
    [cell setChatButtonHandle:^{
        LxmMyQingDanPeopleModel * model = self.dataArr[indexPath.row];
        
        
        NSString * myID = @([LxmTool ShareTool].userModel.userId).stringValue;
        NSMutableArray * idArr = [NSMutableArray arrayWithArray:@[model.robUserId.stringValue,myID]];
        ...
        [LxmNetworking networkingPOST:[LxmURLDefine user_getUserInfoFromHX] parameters:@{@"token":SESSION_TOKEN,@"userIds":ids} returnClass:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            if ([responseObject[@"key"] intValue] == 1) {
                ...
                EaseMessageViewController *chatController = [[EaseMessageViewController alloc] initWithConversationChatter:model.robUserId.stringValue conversationType:EMConversationTypeChat];
