    ...
    if (ISLOGIN) {
        ...
        [LxmNetworking networkingPOST:[LxmURLDefine user_getMyTourInfo] parameters:dic returnClass:[LxmMyTourDetailRootModel class] success:^(NSURLSessionDataTask *task, LxmMyTourDetailRootModel * responseObject) {
            if (responseObject.key.intValue == 1) {
                ...
                self.moneyView.rightTF.text = [NSString stringWithFormat:@"￥%@",self.detailModel.allMoney];
...
//去支付
- (void)bottomBtnClick{
    LxmPayVC * payVC = [[LxmPayVC alloc] initWithTableViewStyle:UITableViewStylePlain type:2];
...
- (void)initFooterView{
    ...
    self.noteTextView.placeholder = @"2个人";
