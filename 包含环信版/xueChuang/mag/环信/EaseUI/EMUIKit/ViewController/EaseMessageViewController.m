...
#import <Hyphenate/EMChatroomManagerDelegate.h>
...
- (void)viewDidLoad {
    ...
    self.chatToolbar.backgroundColor = UIColor.redColor;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didBecomeActive)
                                               ...
                                               object:nil];
    //EaseUIResource.bundle/chat_sender_bg EaseUIResource.bundle/chat_receiver_bg
    
#pragma mark - 气泡位置
    
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"chatsend"] stretchableImageWithLeftCapWidth:15 topCapHeight:15]];
    
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"chatreceive"] stretchableImageWithLeftCapWidth:15 topCapHeight:15]];
    ...
    //创建tableView的头视图
    if (self.conversation.type == EMConversationTypeChat&&self.isDetail) {
        ...
        if (self.detailModel.img&&![self.detailModel.img isEqualToString:@""]) {
            NSArray * arr = [self.detailModel.img componentsSeparatedByString:@","];
            if (arr.count>=1) {
                [imgView sd_setImageWithURL:[NSURL URLWithString:[LxmURLDefine getPicImgWthPicStr:arr.firstObject]] placeholderImage:[UIImage imageNamed:@"whequemoren"] options:SDWebImageRetryFailed];
            }
            
        }else{
            imgView.image = [UIImage imageNamed:@"whequemoren"];
        }
        
        UILabel * titleLab = [[UILabel alloc] initWithFrame:CGRectMake(90, 25, ScreenW - 90-15-100, 20)];
        titleLab.font = [UIFont systemFontOfSize:16];
        titleLab.textColor = BlueColor;
        titleLab.text = self.detailModel.title;
        [headerView addSubview:titleLab];
        UILabel * moneyLab = [[UILabel alloc] initWithFrame:CGRectMake(90, 55, ScreenW - 90-15-100, 20)];
        moneyLab.font = [UIFont systemFontOfSize:15];
        moneyLab.textColor = CharacterDarkColor;
        moneyLab.text = [NSString stringWithFormat:@"%@元/%@",self.detailModel.money,self.detailModel.unit];
        [headerView addSubview:moneyLab];
        
        UIButton * yuedanBut = [[UIButton alloc] initWithFrame:CGRectMake(ScreenW - 15 - 100, 30, 100, 40)];
        [yuedanBut setBackgroundImage:[UIImage imageNamed:@"select"] forState:UIControlStateNormal];
        [yuedanBut setTitle:@"立即约" forState:UIControlStateNormal];
        [yuedanBut addTarget:self action:@selector(yuedan) forControlEvents:UIControlEventTouchUpInside];
        [yuedanBut setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
        yuedanBut.titleLabel.font = [UIFont systemFontOfSize:15];
        yuedanBut.layer.cornerRadius  = 5;
        yuedanBut.layer.masksToBounds = YES;
        [headerView addSubview:yuedanBut];
    }
...
- (void)yuedan{
    if (ISLOGIN) {
        ...
        vc.billId = self.detailModel.billId;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.isSender) {
        model.avatarURLPath = self.myUserModel.headimg;
        model.nickname = self.myUserModel.nickname;
    }else{
        model.avatarURLPath = self.userModel.headimg;
        model.nickname = self.userModel.nickname;
    }
