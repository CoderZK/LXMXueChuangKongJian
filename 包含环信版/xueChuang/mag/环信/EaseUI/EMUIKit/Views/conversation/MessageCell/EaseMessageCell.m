...
+ (void)initialize
{
    ...
    cell.leftBubbleMargin = UIEdgeInsetsMake(8, 13, 8, 10);
    cell.rightBubbleMargin = UIEdgeInsetsMake(8, 5, 8, 20);
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    _statusButton.backgroundColor = [UIColor brownColor];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:[NSString stringWithFormat:@"  %@",model.text ] textFont:self.messageTextFont];
            }
                break;
            case EMMessageBodyTypeImage:
            {
